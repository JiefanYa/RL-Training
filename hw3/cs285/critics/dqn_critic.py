...
class DQNCritic(BaseCritic):
    ...
    def _build(self, q_func):
        ...
        if self.double_q:
            ...
            idx = tf.stack(
                [tf.range(tf.shape(self.obs_t_ph)[0]), tf.cast(tf.argmax(self.q_t_for_tp1, axis=1), tf.int32)], axis=1)
        ...
        # TODO calculate the targets for the Bellman error: DONE
        ...
        target_q_t = self.rew_t_ph + self.gamma * q_tp1 * (1 - self.done_mask_ph) # QUESTION: what's (1-done_mask)
        target_q_t = tf.stop_gradient(target_q_t)

        #####################

        # TODO compute the Bellman error (i.e. TD error between q_t and target_q_t): DONE
        ...
        # TODO these variables should all of the: DONE
